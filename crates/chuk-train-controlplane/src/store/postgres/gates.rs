//! PgStore — [`GateStore`] impl (gates + watchdogs).
...
fn gate_from_row(r: &PgRow) -> Result<GateInfo> {
    Ok(GateInfo {
        scope: r.get("scope"),
...
impl GateStore for PgStore {
    ...
    async fn gates(&self, scope: &str, scope_id: &str) -> Result<Vec<GateInfo>> {
        let rows = sqlx::query(
            "SELECT scope, scope_id, name, expr, action, created_at,
                    tripped, last_value, evaluated_at, detail
             FROM gates WHERE scope = $1 AND scope_id = $2 ORDER BY name",
        )
        .bind(scope)
        .bind(scope_id)
        .fetch_all(&self.pool)
    ...
    async fn metric_history(
        ...
        Ok(rows
            .into_iter()
            .map(|r| MetricObservation { ts: r.get("ts"), value: r.get("value") })
            .collect())
    }
}
